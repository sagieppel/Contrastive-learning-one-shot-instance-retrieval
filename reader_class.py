...
class reader():
    def __init__(self,main_dir,augmentation_level):
        self.data=[]
        self.augmentation_level=augmentation_level # how much to augment the image "low","high","none"
        for sdr in os.listdir(main_dir):
            path1=main_dir+"/"+sdr +"//"
            if not os.path.isdir(path1): continue
            for sdr2 in os.listdir(path1):
                    path2 = path1 + "//" + sdr2 + "//"
                    if not os.path.isdir(path2): continue
                    file_list = []
                    for fl in os.listdir(path2):
                        if not ".jpg" in fl: continue
                        file_list.append(path2 + "//" + fl)
                    if len(file_list)>=2:
                              self.data.append({"name":sdr,"files":file_list})
#########################################################################################################################
    def readbatch(self,ncluster=4,ninst=2,augment=True): # read batch for trainin or testing
         imgs = []
         lbls = []
         names = []
         files_data =[]
             ...
             for  finst in range(ninst):
                    ...
                    if augment and  self.augmentation_level != "none":
                              im,mask=augment_image.augment_image(im,mask=mask,
                                      apply_gaussian=np.random.rand()<0.5,
                                      apply_decolor=np.random.rand()<0.3,
                                      apply_noise=np.random.rand()<0.5,
                                      apply_intensity=np.random.rand()<0.3,
